...
class Particle{
  constructor(canvas, options){
    ...
    this.color  = this.radius > 5 ? "#c5cae9" : "#ffd54f" //this.randomColor()
  }

  randomColor() {
    let colors = ["#c5cae9", "#ffd54f"];
...
function writeText(canvas, context, text){
	var test = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
	var size = 100
	if (test < 600){
		size = 62
	}
	else if (test > 600 && test <900){
		size = 80
	}
